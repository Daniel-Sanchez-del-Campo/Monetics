...
import { AuthService, GastoService, DashboardService, CategoriaService, TicketService } from '../../core/services';
...
export class DashboardComponent implements OnInit {
  ...
  verImagen(gasto: Gasto): void {
    if (gasto.imagenNombre) {
      // Cargar imagen desde el endpoint local del backend
      this.ticketService.obtenerImagen(gasto.imagenNombre).subscribe({
        next: (blob: Blob) => {
          this.imagenAbierta = URL.createObjectURL(blob);
        },
        error: () => {
          // Fallback a Drive URL o Base64
